...
Vagrant.configure("2") do |config|
  ...
  # Every Vagrant development environment requires a box. You can search an/contrib-jessie64for
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo debconf-set-selections <<< 'mysql-server mysql-server/root_password_again password root'
         apt-get install -y apache2 redis-server php mysql-server npm git vim libapache2-mod-php php-mcrypt php-mysql elasticsearch php7.0-mysql php7.0-intl php7.0-curl php7.0-mcrypt php7.0-xml
...
</VirtualHost>" > /etc/apache2/sites-available/OC_p3.conf
sudo a2ensite OC_p3.conf
